...
const Transfers = () => {
	...
	const sDestAccountId = searchParams.get("destAccountId")
	...
	useEffect(() => {
		if (sDestAccountId && sAmount && sRecipient) {
			setIsTransferAllowed(true)
		}
	}, [sDestAccountId, sAmount, sRecipient]);
		...
		>
			...
			<section className={styles.group}>
				...
				{data?.cards.map(card => (
					...
					>
							<img className={styles.card} src={cardPng} />
